...
class Fractal_Headers:

    def headers(self, api_key, partner_id, token=None):
...
class Fractal(Fractal_Headers):
   
    # GET access token function
    def fetch_access_token(self, api_key, partner_id):

        super().headers(api_key, partner_id)

        AUTH_ENDPOINT = "https://7gq3bccyoa.execute-api.eu-west-1.amazonaws.com/v1/token"
        
        auth_response = requests.post(url = AUTH_ENDPOINT, headers=self.headers_auth)
    ...
    def get_bank_trans(self, api_key, partner_id, bankId, companyId, token, acc_id):

        super().headers(api_key, partner_id, token)
        ...
        response_bank_trans = requests.request("GET", BANK_TRANS_URL, headers=self.headers_trans, params=querystring_bank_trans)
...
def main():
    ...
    h1.headers(api_key, partner_id)

    #Function call to fetch JWT authorization token
    auth_code, token = f1.fetch_access_token(api_key, partner_id)

    if auth_code == 201:

    #Function call to retreive banking transactions
        h1.headers(api_key, partner_id, token)
        bank_trans_code, bank_trans_message = f1.get_bank_trans(api_key, partner_id, bankId, companyId, token, accountId)
